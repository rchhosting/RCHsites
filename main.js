...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── THEME ──
  const stored = localStorage.getItem('rch-theme');
  const prefersDark = window.matchMedia('(prefers-color-scheme: dark)');

  function applyTheme(theme) {
    document.documentElement.setAttribute('data-theme', theme);
    localStorage.setItem('rch-theme', theme);
  }
  ...
  // Listen for system changes (only if user hasn't manually toggled)
  prefersDark.addEventListener('change', (e) => {
    if (!localStorage.getItem('rch-theme-manual')) {
  ...
  if (toggleBtn) {
    toggleBtn.addEventListener('click', () => {
      ...
      applyTheme(next);
      localStorage.setItem('rch-theme-manual', 'true');
    });
  }
  ...
  if (gc) {
    ...
    function getGridColor() {
      ...
      return theme === 'light' ? 'rgba(45,106,196,0.05)' : 'rgba(74,143,231,0.04)';
    }
    ...
    function drawGrid() {
      ...
      gx.strokeStyle = getGridColor(); gx.lineWidth = 0.6;
